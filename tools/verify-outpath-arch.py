...
E_MACHINE = {0x3E: "x86_64", 0xB7: "aarch64", 0x28: "arm", 0xF3: "riscv"}
# Big NARs are slow to fetch and no more informative than small ones.
...
def arch_of(digest):
    ...
    info = narinfo(digest)
    if not info or int(info.get("FileSize", "0")) > MAX_FILE_BYTES:
        return None
    try:
...
def main():
    ...
    ap.add_argument("--artifact", required=True, help="an outpaths-<system>.json")
    ...
    with cf.ThreadPoolExecutor(THREADS) as ex:
        ...
        for (attr, ver, digest), arch in zip(
            pairs, ex.map(lambda p: arch_of(p[2]), pairs[: args.count * 5])
        ):
            if arch:
