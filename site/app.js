...
const COPY_ICON = `<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4">
...
function cmdBlock(text, caption) {
  return (
    (caption ? `<div class="capt">${esc(caption)}</div>` : "") +
...
function pinsFor(offset) {
  if (!pinsByOffset) {
    ...
    for (const l of pinsByOffset.values())
...
const pinLinks = (pins) =>
  pins
    .map(
      ([a, v]) =>
        `<div><a href="#" class="attr" data-attr="${esc(a)}">${esc(a)}</a> <span class="muted">${esc(v)}</span></div>`,
    )
    .join("");
...
function renderStats() {
  ...
  const first = revisions[0].date,
    last = revisions[revisions.length - 1].date;
  $("stats").textContent =
...
function renderSearch() {
  ...
  out.textContent = "";
  if (!index) {
    status.textContent = "Loading versions.json…";
    return;
  }
  if (!q) {
    status.textContent = "";
  ...
  out.innerHTML = hits
    .map(
      (a) =>
       ...
       <span class="muted">· ${Object.keys(index.attrs[a]).length} versions</span></div>`,
    )
    .join("");
}
...
function selectTab(view) {
  ...
  for (const v of ["packages", "revisions", "releases"])
    $(v).hidden = v !== view;
}

document
  .querySelectorAll("nav button")
  .forEach((b) => b.addEventListener("click", () => selectTab(b.dataset.view)));
...
document.addEventListener("click", (e) => {
  ...
  if (more) {
    more.previousElementSibling.innerHTML = pinLinks(
      pinsFor(Number(more.dataset.offset)),
    );
    more.remove();
    return;
  }
  ...
  if (pkg) {
    renderPackage(pkg.dataset.attr);
    return;
  }
  const attr = e.target.closest("a.attr");
  if (attr) {
    e.preventDefault();
    goToPackage(attr.dataset.attr);
    return;
  }
  const rev = e.target.closest("a.rev");
  if (rev) {
    e.preventDefault();
    goToRevision(Number(rev.dataset.offset));
    return;
  }
...
async function load() {
  ...
  try {
    [revisions, releases] = await Promise.all([
      json("revisions.json"),
      json("releases.json"),
    ]);
    renderRevisions();
